...
class Wall(Building):
    def __init__(self, position, V):
        ...
        self.health = self.max_health
        self.explosion = 200 if self.level >= 3 else 0
        self.explosion_radius = 2 if self.level >= 3 else 0
        self.type = 'wall'

    def isInExplosionRadius(self, troop):
        r = abs(self.position[0] - troop.position[0])
        c = abs(self.position[1] - troop.position[1])
        
        if(r <= self.explosion_radius and c <= self.explosion_radius):
            return True
        return False
    ...
    def explode(self, King):
        ...
        for troop in troops:
            if self.isInExplosionRadius(troop):
                troop.deal_damage(self.explosion)

        if self.isInExplosionRadius(King):
            King.deal_damage(self.explosion) 
