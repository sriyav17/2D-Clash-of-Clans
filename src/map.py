...
def printMap(V):
    ...
    for stealtharch in stealth_archers:
        if(stealtharch.check_invisible() == True):
            continue

        a= 2*stealtharch.position[0]
        b = 2*stealtharch.position[1]
        map_matrix[a][b] = archColor(stealtharch) + Fore.BLACK + 'AR'
        map_matrix[a+1][b] = archColor(stealtharch) + Fore.BLACK + '  '
        map_matrix[a][b+1] = archColor(stealtharch) + Fore.BLACK + 'CH'
